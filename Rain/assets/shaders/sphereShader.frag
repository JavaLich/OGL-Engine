...
float noise(vec4 position, int octaves,float frq, float persistence){
	float total;
	float frequency = frq;
	float amplitude = 1;
	float maxValue = 0.0;
...
void main(){
	...
	vec4 sPosition = position * realRadius;
	float s= 0.1;
	float frequency = 0.000001;
	float t1 = snoise(sPosition*frequency)-s;
	float t2 = snoise((sPosition+realRadius)*frequency)-s;
